...
print("LOGGING TO FILE : 'log/tune_database_{}.log".format(ts))
...
def get_original_knob_details():
    global ORIGINAL_KNOB_DETAILS
    return ORIGINAL_KNOB_DETAILS
...
def initialize_knobs(knobs_config, num, keys=[], lower_dim = None):
    ...
    ORIGINAL_KNOB_DETAILS = json.loads(json.dumps(KNOB_DETAILS)) # Make deep copy of original knob details

    # THIS CONVERTS ALL KNOBS TO LOWER DIM AS SOON AS INITIALIZED
    # print("KNOB DETAILS BEFORE TRANSFORM:")
    # print(KNOB_DETAILS)
    if lower_dim and lower_dim["enabled"]:
        low_level_project_input(lower_dim["target_dim"], KNOBS, KNOB_DETAILS)
        print("KNOB DETAILS AFTER TRANSFORM:")
        print(KNOB_DETAILS)
    return KNOB_DETAILS
